...
def plot_whomp_random(X_toy, subgroup_number=2):
    ...
    plt.title(f"Example of WHOMP Random on a mixture of {subgroup_number + 1} distributions")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.xlim(-25, 25)
    plt.ylim(-25, 25)
...
def plot_whomp_matching(X_toy, subgroup_number=2, threshold=0.0000001):
    ...
    plt.title(f"Example of WHOMP Matching a mixture of {subgroup_number + 1} distributions")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.xlim(-25, 25)
    plt.ylim(-25, 25)
...
def plot_pocock_simon(X_toy, subgroup_number=2):
    ...
    for i in range(subgroup_number + 1):  # Pocock & Simon is handling 3 distributions
        plt.scatter(X_toy[np.where(PS_index == i)[0], 0], X_toy[np.where(PS_index == i)[0], 1])
    
    plt.title(f"Example of Pocock & Simon's on a mixture of {subgroup_number + 1} distributions")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.xlim(-25, 25)
    plt.ylim(-25, 25)
...
def NPI_experiment(X, subgroup_number_list, repetition):
    ...
    return random_error_ave, PS_error_ave, anticluster_error_ave, WHOMP_error_ave, \
...
def WHOMP_MNIST_experiment(tsne, y_subset, subgroup_number, repetition):
    """
    Run entropy experiment on MNIST data with different partitioning methods.
    """
    random_ent = []
    PS_ent = []
    anticluster_ent = []
    ...
    return random_ent, PS_ent, anticluster_ent, WHOMP_ent
